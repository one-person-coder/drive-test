...
import { Cloud, Download, Rocket } from "lucide-react";
...
const ResumeCloud = () => {
  ...
  useEffect(() => {
    const getDownloadLink = async () => {
      const movie = await fetch(getApiUrl(pathname, "cloud"));
  ...
  return (
    <>
      {isLoading ? (
        ...
        link && (
          <Card className="bg-zinc-800/30 border-zinc-700/50 hover:border-cyan-500/50 transition-all group">
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-zinc-400">
                      Fast cloud storage with resume capability
                    </p>
                  </div>
                </div>
                <Button
                  className="bg-gradient-to-r from-cyan-600 to-fuchsia-600 hover:from-cyan-700 hover:to-fuchsia-700 text-white"
                  asChild
                >
